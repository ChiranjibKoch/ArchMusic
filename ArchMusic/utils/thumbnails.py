...
def _gradient_band(
    w: int, h: int, c1: tuple, c2: tuple, alpha1: int = 210, alpha2: int = 210
) -> Image.Image:
    arr = np.zeros((h, w, 4), dtype=np.uint8)
    for y in range(h):
        t = y / max(h - 1, 1)
...
def _style_cinema(base, title, duration, views, channel, label):
    ...
    d.line(
        [(W // 2 - 160, start_y + total_h + 14), (W // 2 + 160, start_y + total_h + 14)],
        fill=c1,
        width=3,
    )
...
async def _fetch_thumb(videoid: str, thumb_url: str = None) -> str:
    ...
    urls += [
        ...
        f"http://img.youtube.com/vi/{videoid}/hqdefault.jpg",
    ]
...
async def _fetch_yt_meta(videoid: str, prefetched: dict = None) -> dict:
    if prefetched:
        return {
            "title":    re.sub(r"\W+", " ", prefetched.get("title", "Unknown")).title(),
            "duration": prefetched.get("duration_min", "Unknown"),
            "views":    prefetched.get("views", "Unknown"),
            "channel":  prefetched.get("channel", "Unknown"),
            "thumb_url": prefetched.get("thumbnail"),
        }
    ...
    for result in (await results.next())["result"]:
        ...
        try:
            data["thumb_url"] = result["thumbnails"][0]["url"].split("?")[0]
        except Exception:
            data["thumb_url"] = None
    return data


async def gen_thumb(videoid: str, prefetched: dict = None) -> str:
    try:
        out_path = f"cache/{videoid}.jpg"
        if os.path.isfile(out_path):
            return out_path
        meta = await _fetch_yt_meta(videoid, prefetched=prefetched)
        thumb_path = await _fetch_thumb(videoid, meta.get("thumb_url"))
        if not thumb_path:
...
async def gen_qthumb(videoid: str, prefetched: dict = None) -> str:
    try:
